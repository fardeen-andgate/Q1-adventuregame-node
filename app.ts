...
const battle = async () => {
  console.log(
    chalk.bold.green(`A thrilling battle against ${o1.name} begins...`)
  );
  while (p1.fuel > 0 && o1.fuel > 0) {
    ...
    if (actions.opt === "Attack") {
      ...
      if (p1.drinkPortions > 0) {
        ...
        console.log(
          chalk.bold.green(
            `You drank a potion and feel re-energized! (${p1.drinkPortions} left)`
          )
        );
      } else {
...
if (opponent.select === "Skeleton") {
  ...
  console.log(
    chalk.bold.red(
      "You are facing an Assassin! Beware of their stealthy attacks."
    )
  );
  battle();
} else if (opponent.select === "Zombie") {
  console.log(
    chalk.bold.green("You are facing a Zombie! Beware of their bite")
  );
  battle();
}
